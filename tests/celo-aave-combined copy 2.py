...
parser = argparse.ArgumentParser(description='Supply, Withdraw, or Set Collateral for CELO on Aave')
parser.add_argument('--action', type=str, required=True, choices=['supply', 'withdraw', 'set-collateral'], 
                    help='Action to perform: "supply", "withdraw", or "set-collateral"')
parser.add_argument('--amount', type=float, default=0, 
                    help='Amount of CELO to supply/withdraw (e.g., 0.05). Use 0 for withdraw all. Not used for set-collateral.')
...
def main():
    ...
    if action == "supply":
        ...
        print(f"Invalid action: {action}. Use 'supply', 'withdraw', or 'set-collateral'.")
